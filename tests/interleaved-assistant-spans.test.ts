...
test('emitAssistantTextSpan: text lands in gen_ai.output.messages as a text part', async () => {
  const { tracer, exporter, provider } = setupTracer();
  const parent = tracer.startSpan('chat');

  emitAssistantTextSpan(tracer, parent, {
    conversationId: 'conv-1',
    text: 'hello world',
  });
  ...
  const span = exporter
    .getFinishedSpans()
    .find((s) => s.attributes[ATTR.OPERATION_NAME] === OP.ASSISTANT_TEXT);
  assert.ok(span);
