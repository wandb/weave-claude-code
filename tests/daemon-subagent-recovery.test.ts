...
// Production registers this via NodeTracerProvider.register(); the test injects
// a BasicTracerProvider, so set it up here or context.with won't propagate.
...
function makeDaemon(tracer: unknown) {
  const logFile = path.join(os.tmpdir(), `wcp-subrecover-${process.pid}.log`);
  const d = new GlobalDaemon('/tmp/unused-subrecover.sock', logFile, 'e/p', 'k', 'https://x', false, 'claude code');
...
test('SubagentStop with no tracker (post-restart) recovers the subagent: turn -> invoke_agent -> chat with tokens', async () => {
  ...
  try {
    ...
    const chat = spans.find(
      (s) => s.attributes['gen_ai.operation.name'] === 'chat' && s.parentSpanContext?.spanId === subInvoke.spanContext().spanId,
    );
    assert.ok(chat, `expected the subagent chat span nested under the subagent invoke_agent span; got: ${names}`);
    ...
    assert.equal(subInvoke.parentSpanContext?.spanId, turn.spanContext().spanId, 'subagent invoke_agent nests under the reconstructed turn');
  } finally {
...
test('recovery reuses an already-open turn span instead of creating a spurious second turn', async () => {
  ...
  try {
    ...
    const subInvoke = spans.find((s) => s.attributes['gen_ai.agent.name'] === 'Explore');
    assert.ok(subInvoke, 'recovered subagent invoke_agent span present');
    assert.equal(subInvoke.parentSpanContext?.spanId, turns[0].spanContext().spanId, 'subagent nests under the pre-existing turn');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
    await provider.shutdown();
  }
