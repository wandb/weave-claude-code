...
// general-purpose subagent). When it does, the grandchild's `invoke_agent` span
// must nest under its spawning subagent's `invoke_agent` span — the same way a
// top-level subagent nests under the turn. Previously the daemon only created an
// invoke_agent span + tracker for Agent-tool calls from the MAIN agent, so a
// subagent-initiated dispatch was never tracked and the grandchild orphaned onto
// the turn span ("no tracker matches ...; creating orphan").
//
// Drives the real routeEvent entry point (as turn-span-integration does) so the
// PreToolUse/SubagentStart correlation runs end to end, and asserts the public
// contract: the grandchild's exported invoke_agent span parents under its
// spawning subagent and carries no orphan reason.
