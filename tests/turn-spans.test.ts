...
//   1. integration identity propagated to every span (turn, chat, tool)
...
//   4. system-instructions capture: buffering, dedup, and per-turn propagation
...
// SessionStart and sets them on the session's Conversation; the SDK propagates
// them to every span it emits, and routeEvent re-activates the conversation for
...
// re-activation is exercised.

test('integration identity propagates weave.integration.* to every span (turn, chat, tool)', async () => {
  ...
  try {
    ...
    // The per-event conversation re-activation must not disturb the trace tree: the
...
// propagates them to every turn root as `gen_ai.system_instructions`. The hook
...
// production does) so buffering, draining, dedup, and per-turn propagation are all
...
test('propagates system instructions to every turn root (no session span to hang them on)', async () => {
  const exporter = await initWeaveInMemory();
  exporter.reset();
