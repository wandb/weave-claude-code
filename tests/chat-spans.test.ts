...
// Chat-span behavior for assistant responses: each response is a single `chat`
// span whose ordered `gen_ai.output.messages` parts carry the response's blocks
// (thinking / text / tool_use) in transcript order. Tool executions are sibling
// spans under the owning agent turn. This file covers:
//
//   - contentBlocksToParts: the pure formatting layer (block -> ordered part).
//   - Handler integration (via routeEvent): Stop reconstructs response spans,
//     SessionEnd recovers an interrupted response, and tools stay under the turn.
...
// End-to-end output-part interleave and turn-owned tool spans are covered by
// the handler and split-line tests below.
...
test('handlers: Stop emits multiple tool-calling responses once under the turn', async () => {
  ...
  try {
    ...
    // Second response with its own tool_use.
