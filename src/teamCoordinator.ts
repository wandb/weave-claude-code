...
type PendingBase = {
  session: Session; call: TracedAgent;
...
export class TeamCoordinator {
  ...
  async postOutcome(call: TracedAgent, outcome: ToolResult): Promise<TeamPostUpdate> {
  ...
  registerIdle(
    ...
    deferAgentOutcome(call, { ok: true, output: null });
    this.calls.push({
  ...
  private async finish(
    ...
    finishAgentCall(pending.session.calls, pending.call, {
      outcome: { ok: true, output: emitted.text ?? output },
    });
